...
contract CreateSubscription is Script {
    ...
    function createSubscription(address vrfCoordinator, address account) public returns (uint256, address) {
        console2.log("creating subsription on chain Id: ", block.chainid);
...
contract FundSubscription is Script, CodeConstants {
        ...
        public
    {
        ...
        if (block.chainid == LOCAL_CHAINID) {
            ...
            vm.startBroadcast(account);
            LinkToken(linkToken).transferAndCall(vrfCoordinator, FUND_AMOUNT * 50, abi.encode(subscriptionId));
            vm.stopBroadcast();
        }
    }
...
contract AddConsumer is Script {
    ...
    function addConsumer(address contractToAddToVrf, address vrfCoordinator, uint256 subId, address account) public {
        console2.log("address of contract to be added:", contractToAddToVrf);
    ...
    function run() external {
        address mostRecentlyDeployed = DevOpsTools.get_most_recent_deployment("ShredderNft", block.chainid);
